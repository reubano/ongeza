...
class GitUtilsTests(BumpTestCase):

    def test_get_current_git_tag(self):
        expected = "3.0.1"

        tag = bump.git_utils.get_current_git_tag()
    ...
    def test_sh_shell_methods(self):
        describe = bump.shell_utils.sh("git describe")
        self.assertIsInstance(describe, basestring)

        describe = bump.shell_utils.shell("git describe").output()[0]
        self.assertIsInstance(describe, basestring)
    ...
    def test_get_git_tags(self):
        tags = bump.git_utils.get_git_tags()
    ...
    @patch("bump.git_utils.has_unstaged_changes")
    @patch("bump.git_utils.has_uncommitted_changes")
    @patch("bump.git_utils.git_is_clean")
    def test_ensure_clean_index_valid_returns(self, git_is_clean, has_uncommitted_changes, has_unstaged_changes):
        ...
        # TODO
        rv = bump.git_utils.ensure_clean_index(skip_interactive=True)
        self.assertTrue(rv)


    @patch("bump.git_utils.git_is_clean")
    @patch("bump.git_utils.git_diff_files")
    @patch("bump.shell_utils.stderr")
    def test_ensure_clean_index_invalid_fails(
        ...
        with self.assertRaises(SystemExit):
            bump.git_utils.ensure_clean_index(skip_interactive=True)
    ...
    @patch("bump.git_utils.git_stash")
    @patch("bump.git_utils.git_is_clean")
    @patch("bump.git_utils.git_diff_files")
    @patch("bump.shell_utils.input")
    def test_ensure_clean_index_invalid_interactive_works(
        ...
        rv = bump.git_utils.ensure_clean_index(False, callback)
    ...
    def test_ensure_clean_index_invalid_interactive_fails(
        ...
        with self.assertRaises(SystemExit):
            bump.git_utils.ensure_clean_index(False)
